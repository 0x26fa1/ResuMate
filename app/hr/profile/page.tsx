...
export default function ProfilePage() {
  ...
  const handleSaveJobPreferences = async (e: React.FormEvent) => {
    ...
    if (error) {
      console.error(error)
      alert("❌ Error updating job preferences.")
    } else {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {userType === "jobseeker" && (
              <Card>
                ...
                <CardContent>
                  {editingJobPref ? (
                    <form onSubmit={handleSaveJobPreferences} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                      ...
                      <div className="col-span-2 flex justify-end space-x-2 mt-4">
                        <Button type="button" variant="outline" onClick={() => setEditingJobPref(false)} disabled={busy}>
                          Cancel
                        </Button>
                        <Button type="submit" disabled={busy}>Save Changes</Button>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Work Type</label>
                        <p className="mt-1 text-sm text-gray-900">{Array.isArray(profile.work_type) ? profile.work_type.join(", ") : (profile.work_type || "Not set")}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-500">Availability</label>
                        <p className="mt-1 text-sm text-gray-900">{profile.availability || "Not set"}</p>
                      </div>
                    </div>
                  )}
                </CardContent>
            ...
            {userType === "hr" && (
              <Card>
                ...
                <CardContent>
                  {editingCompany ? (
                    <form onSubmit={handleSaveCompanyInfo} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                      ...
                      <div className="col-span-2 flex justify-end space-x-2 mt-4">
                        <Button type="button" variant="outline" onClick={() => setEditingCompany(false)} disabled={busy}>
                          Cancel
                        </Button>
                        <Button type="submit" disabled={busy}>Save Changes</Button>
                      </div>
